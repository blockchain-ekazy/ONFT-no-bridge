...
import ABI_ONFT from "../../assets/abis/ONFT.json";
...
export const TraverseModal = ({
  ...
  const getAvailableChains = () => {
    return traversingCards[0].addresses.map((a_: any) => {
      return a_.chainId;
    });
  };
  ...
  const onClickTraverse = async () => {
    ...
    try {
      ...
      console.log(Math.round(Number(crossGas[0]) + additional_pay));
      ...
      let tx = await traverseContract.traverse(
        destChainId,
        contractAddress,
        destcontracts.find((a_: any) => a_.chainId == selectedChain).address,
        selectedCards.map((card: any) => card.tokenId),
